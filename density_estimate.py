...
for i in range(n_bin):
    ...
    if z1 < 4:
        C_ID = (no_ID + yes_ID) / yes_ID
    elif z1 < 4.5:
        C_ID = 1  # /0.92 # The C19 sample is complete up to 92% at 4<z<4.5
    else:
        C_ID = 1
    ...
    if z1 < 4:
        C_spect = obj_inbin / with_spectrum_inbin
    else:
        C_spect = 1
    ...
    if z1 < 4.5:
        ...
        percent_unc[i, :] = gehrels(count_inbin) / np.array(count_inbin)
...
ax.errorbar(
    6, 1.10 / 200, xerr=0.5, yerr=[[0.91 / (2 * gamma * gamma)], [2.53 / (2 * gamma * gamma)]], fmt='D',
    color='magenta',
    elinewidth=0.7, markersize=3, capsize=2, capthick=0.7
)
ax.fill_between(z_RLF, 10 ** RLF_low * 2 * gamma ** 2, 10 ** RLF_high * 2 * gamma ** 2, color='#5681b9',
                label=r'$\Gamma=10$', alpha=0.5)
...
ax.fill_between(z_RLF, low_fraction * 2 * gamma ** 2, high_fraction * 2 * gamma ** 2, color='#5681b9', alpha=0.5,
                label=r"$\Gamma=10$")
ax.fill_between(z_RLF, low_fraction * low_g, high_fraction * low_g, color='#002A7F', alpha=0.5, label=r"$\Gamma=5$")
plt.yscale('log')
ax.errorbar([6], [0.119], xerr=[[0.25], [0.25]], yerr=[[0.053], [0.053]], fmt='D', color='orange', markersize=6,
            elinewidth=0.9, capsize=2, capthick=0.9, label="Liu (2021)")  # Liu 2021
ax.errorbar(5, 0.071, xerr=[[0.3], [0.4]], yerr=[[0], [0.05]], fmt='x', color='red', markersize=8, lolims=True,
            elinewidth=0.9, capsize=2, capthick=0.9, label="Yang (2016)")  # Yang 2016
ax.errorbar([5.95], [0.081], xerr=[[0.45], [0.45]], yerr=[[0.032], [0.05]], fmt='*', color='blue', markersize=8,
            elinewidth=0.9, capsize=2, capthick=0.9, label="Bañados (2015)")  # Banados 2015
ax.plot([1.53 for _ in range(11)], np.arange(0.10, 0.21, 0.01), alpha=0.75, lw=8, solid_capstyle='round',
        color='forestgreen', zorder=1, label='local')
...
plt.ylim([0.01, 1])
plt.xlim([1.5, 6.5])
leg = ax.legend(loc='lower left', fontsize='x-small', frameon='False', framealpha=0, ncol=2, borderpad=0.1,
